...
/* calclulo-output-tools-floating-v1.js
   Plain JS (no <script> tags). Paste into a .js file or run after your calculators are mounted.
*/
...
(function () {
  ...
  // ---------- small config ----------
  const UI_GAP = 8;            // gap in px when placing outside host
  const INSIDE_PAD = 76;       // padding-right added to host when UI placed inside
  const UI_WIDTH = 56;         // expected UI width (approx)
  const UI_HEIGHT = 120;       // expected UI height (approx)

  // ---------- inline icons ----------
  const COPY_SVG = `<svg viewBox="0 -960 960 960" width="20" height="20" fill="currentColor" aria-hidden="true"><path d="M360-240q-33 0-56.5-23.5T280-320v-480q0-33 23.5-56.5T360-880h360q33 0 56.5 23.5T800-800v480q0 33-23.5 56.5T720-240H360Zm0-80h360v-480H360v480ZM200-80q-33 0-56.5-23.5T120-160v-560h80v560h440v80H200Zm160-240v-480 480Z"/></svg>`;
  const DOWNLOAD_SVG = `<svg viewBox="0 -960 960 960" width="20" height="20" fill="currentColor" aria-hidden="true"><path d="M480-320 280-520l56-58 104 104v-326h80v326l104-104 56 58-200 200ZM240-160q-33 0-56.5-23.5T160-240v-120h80v120h480v-120h80v120q0 33-23.5 56.5T720-160H240Z"/></svg>`;

  // ---------- PDF builder (same simple single-page builder) ----------
  function escapePdfString(s) { return String(s || '').replace(/\\/g, '\\\\').replace(/\(/g, '\\(').replace(/\)/g, '\\)'); }
  ...
  // ---------- inject CSS once (for floating UI) ----------
  (function injectCss() {
    if (document.getElementById('calclulo-floating-ui-styles')) return;
    const css = `
      .calclulo-ui {
        position: absolute;
        display:flex;
        ...
        align-items:center;
        padding:12px;
        background:transparent; /* transparent so underlying content shows */
        pointer-events:auto;
        transition:opacity 180ms ease;
        opacity:0;
      }
      .calclulo-ui.visible { opacity:1; }
      .calclulo-btn {
        all:unset;
        display:grid;
        place-items:center;
        width:44px;height:44px;border-radius:10px;
        background:white;border:1px solid #e6e6e9;color:#0f1724;
        box-shadow:0 6px 18px rgba(15,23,36,0.06);
        cursor:pointer;
      }
      .calclulo-btn:hover { transform:none; background:#f7fafc; }
      .calclulo-icon { width:20px;height:20px; display:block; }
      .calclulo-menu {
        position:absolute;
        right:70px;
        top:50%;
      ...
      .calclulo-menu.open { opacity:1; pointer-events:auto; }
      .calclulo-option { all:unset; padding:10px 12px; border-radius:8px; cursor:pointer; font-size:14px; color:#0f1724; text-align:left; }
      .calclulo-option:hover { background: rgba(15,23,36,0.03); }
      .calclulo-toast { position: absolute; right:78px; top:8px; background:#111827;color:white;padding:8px 10px;border-radius:8px;font-size:13px; opacity:0; transition:opacity 160ms ease; pointer-events:none; }
      .calclulo-toast.show { opacity:1; }
      @media (max-width:520px) { .calclulo-menu { right:62px; min-width:140px; } .calclulo-btn { width:40px;height:40px; } .calclulo-toast { right:72px; } }
    `;
    const s = document.createElement('style');
    s.id = 'calclulo-floating-ui-styles';
    s.textContent = css;
    document.head.appendChild(s);
  })();

  // ---------- create floating UI for a host -->
  function createFloatingUIFor(host) {
    // if UI already exists, return it
    if (host._calcluloFloating) return host._calcluloFloating;

    // create container appended to body
    const node = document.createElement('div');
    node.className = 'calclulo-ui';
    node.setAttribute('aria-hidden', 'false');
    node.setAttribute('data-for', '');
    node.style.zIndex = 99999;
    node.style.pointerEvents = 'auto';

    // inner HTML
    node.innerHTML = `
      <button class="calclulo-btn calclulo-copy" title="Copy results" type="button"><span class="calclulo-icon">${COPY_SVG}</span></button>
      <div class="calclulo-download">
        <button class="calclulo-btn calclulo-download-btn" title="Download results" type="button" aria-expanded="false"><span class="calclulo-icon">${DOWNLOAD_SVG}</span></button>
        <div class="calclulo-menu" role="menu" aria-hidden="true">
          <button class="calclulo-option" data-type="pdf" type="button">Save as PDF</button>
          <button class="calclulo-option" data-type="csv" type="button">Download CSV</button>
          <button class="calclulo-option" data-type="txt" type="button">Download TXT</button>
        </div>
      </div>
      <div class="calclulo-toast" aria-hidden="true"></div>
    `;
    document.body.appendChild(node);
    host._calcluloFloating = node;
    node.dataset.for = '';

    // elements
    const copyBtn = node.querySelector('.calclulo-copy');
    const downloadBtn = node.querySelector('.calclulo-download-btn');
    const menu = node.querySelector('.calclulo-menu');
    const options = node.querySelectorAll('.calclulo-option');
    const toast = node.querySelector('.calclulo-toast');

    // extract text function (prefer .output-content)
    function extractText() {
      const content = host.querySelector('.output-content');
      const text = content ? content.innerText : host.innerText;
    ...
    function showToast(msg) {
      ...
      setTimeout(() => { toast.classList.remove('show'); toast.setAttribute('aria-hidden', 'true'); }, 1200);
    }
      ...
      opt.addEventListener('click', (ev) => {
        ...
        try {
          if (type === 'txt') {
            ...
            triggerDownloadBlob('results.csv', new Blob([makeCsvFromText(txt)], { type: 'text/csv;charset=utf-8' }));
    ...
    // store cleanup
    host._calcluloFloatingCleanup = function () {
      try { document.removeEventListener('click', outsideClick); } catch (e) {}
      try { node.remove(); } catch (e) {}
      delete host._calcluloFloating;
      delete host._calcluloFloatingCleanup;
  ...
  // ---------- position logic ----------
  function positionFloatingUI(host) {
    const ui = host._calcluloFloating;
    if (!ui) return;
    const rect = host.getBoundingClientRect();
    const vw = document.documentElement.clientWidth;
    const vh = document.documentElement.clientHeight;

    // measure UI (if not yet visible, temporary show to measure)
    ui.style.left = '-9999px';
    ui.style.top = '-9999px';
    ui.classList.add('visible'); // make visible for measurement
    const uiRect = ui.getBoundingClientRect();

    // decide place: prefer outside right if enough space
    const spaceRight = vw - rect.right;
    const placeOutside = (spaceRight >= uiRect.width + UI_GAP);

    if (placeOutside) {
      // place outside to the right (absolute coords relative to document)
      const left = window.pageXOffset + rect.right + UI_GAP;
      const top = window.pageYOffset + rect.top + Math.max(8, (rect.height - uiRect.height) / 2);
      ui.style.position = 'absolute';
      ui.style.left = left + 'px';
      ui.style.top = top + 'px';
      // ensure host padding not modified
      if (host.dataset._calcluloAddedPad === '1') {
        host.style.paddingRight = host.dataset._calcluloOrigPad || '';
        delete host.dataset._calcluloAddedPad;
        delete host.dataset._calcluloOrigPad;
      ...
      // place inside host top-right (but add padding so content wont be hidden)
      const left = window.pageXOffset + rect.left + Math.max(0, rect.width - uiRect.width - 12);
      const top = window.pageYOffset + rect.top + 8;
      ui.style.position = 'absolute';
      ui.style.left = left + 'px';
      ui.style.top = top + 'px';

      // add padding-right to host if not opted out
      if (host.dataset.calcluloNoPad !== '1' && host.dataset._calcluloAddedPad !== '1') {
        // store original inline style
        host.dataset._calcluloOrigPad = host.style.paddingRight || '';
        host.style.paddingRight = (parseFloat(host.dataset._calcluloOrigPad || '0') + INSIDE_PAD) + 'px';
        host.dataset._calcluloAddedPad = '1';
      }
    }
    // small safety to ensure visible
    ui.classList.add('visible');
  }

  // ---------- attach / init for each host ----------
  function attachToHost(host) {
    if (!host || host._calcluloFloating) return;
    const ui = createFloatingUIFor(host);
    if (!ui) return;
    positionFloatingUI(host);

    // reposition on window resize/scroll and host resize
    const onWin = throttle(() => positionFloatingUI(host), 120);
    window.addEventListener('resize', onWin, { passive: true });
    window.addEventListener('scroll', onWin, { passive: true });

    // ResizeObserver for host layout changes
    let ro;
    try {
      ro = new ResizeObserver(throttle(() => positionFloatingUI(host), 80));
      ro.observe(host);
    } catch (e) { ro = null; }

    // store cleanup to remove listeners when host removed
    const cleanup = function () {
      try { window.removeEventListener('resize', onWin); window.removeEventListener('scroll', onWin); } catch (e) {}
      try { if (ro) ro.disconnect(); } catch (e) {}
      if (typeof host._calcluloFloatingCleanup === 'function') host._calcluloFloatingCleanup();
    };
    host._calcluloFloatingDetach = cleanup;

    // observe host removal to cleanup UI
    ...
    // observe host.parentNode if exists
    if (host.parentNode) parentObserver.observe(host.parentNode, { childList: true });
    host._calcluloParentObserver = parentObserver;
  }
  ...
  // ---------- scan initial .output-group nodes ----------
  function initAll() {
    const nodes = Array.from(document.querySelectorAll('.output-group'));
    nodes.forEach(host => {
      attachToHost(host);
    });
  }

  // ---------- global observer for newly added groups or moved nodes ----------
      ...
      for (const m of mutations) {
        ...
        if (m.addedNodes && m.addedNodes.length) {
          m.addedNodes.forEach(n => {
            ...
            if (n.matches && n.matches('.output-group')) attachToHost(n);
            // nested
            const nested = n.querySelectorAll && n.querySelectorAll('.output-group');
            if (nested && nested.length) nested.forEach(g => attachToHost(g));
          });
        }
        // attribute changes could reposition (if class/style changed)
        if (m.type === 'attributes' && m.target && m.target.matches && m.target.matches('.output-group')) {
          // reposition UI
          try { positionFloatingUI(m.target); } catch (e) {}
        }
      }
    });
    globalObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['style', 'class'] });
  }

  // ---------- public API ----------
  window.calcluloOutputTools = window.calcluloOutputTools || {};
  window.calcluloOutputTools.attachTo = function (element) { attachToHost(element); positionFloatingUI(element); };
  window.calcluloOutputTools.init = function () { initAll(); startGlobalObserver(); };
  window.calcluloOutputTools.destroy = function () {
    if (globalObserver) { globalObserver.disconnect(); globalObserver = null; }
    document.querySelectorAll('.output-group').forEach(h => {
      try { if (h._calcluloFloatingDetach) h._calcluloFloatingDetach(); } catch (e) {}
      try { if (h._calcluloParentObserver) h._calcluloParentObserver.disconnect(); } catch (e) {}
    });
  };

  // ---------- initial run ----------
  // run after a microtask so other scripts that run on DOMContentLoaded finish
  setTimeout(() => { window.calcluloOutputTools.init(); }, 30);
